...
class RealisticTransductiveCollateFunction(object):
    """`Collate_fn` for Realistic Transductive Few-Shot dataloader.

    For realistic transductive setting where:
    - Support set: k_way classes with n_shot samples each
    - Query set: k_eff classes (subset of k_way) with q_total samples
    
    This collate function does NOT assume uniform class distribution.
    """
    ...
    def __init__(
        ...
        super(RealisticTransductiveCollateFunction, self).__init__()
        try:
        ...
        self.support_size = k_way * n_shot

    def method(self, batch):
        """
        Apply transforms and augmentations on a realistic transductive batch.
        
        Batch structure: [support_samples (k_way * n_shot)] + [query_samples (q_total)]
        
        Args:
            batch (list of tuple): A batch returned by RealisticTransductiveSampler.
            
        Returns:
            tuple: (images, global_labels)
                - images: [b', c, h, w] where b' = support_size * times + q_total * times_q
                - global_labels: [1, support_size * times + q_total * times_q]
        """
        try:
            ...
            support_images = images[:self.support_size]
            query_images = images[self.support_size:]
            support_labels = labels[:self.support_size]
            query_labels = labels[self.support_size:]
            
            support_images = support_images * self.times
            query_images = query_images * self.times_q
            support_labels = support_labels * self.times
            query_labels = query_labels * self.times_q
            
            support_images = [self.trfms_support(img) for img in support_images]
            query_images = [self.trfms_query(img) for img in query_images]
            
            images = torch.stack(support_images + query_images)
            
            global_labels = torch.tensor(
                support_labels + query_labels, dtype=torch.int64
            ).unsqueeze(0)
