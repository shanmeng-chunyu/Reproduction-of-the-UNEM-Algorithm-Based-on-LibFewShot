...
from .realistic_sampler import get_realistic_transductive_sampler
...
def get_sampler(dataset, few_shot, distribute, mode, config):
    if few_shot:
        ...
        print(f"Sampler debug: classifier_name={classifier_name}, use_fgfl={use_fgfl}, use_realistic={use_realistic}")

        if use_realistic:
            # 如果你在文件顶部已经导入了，这里的局部导入可以删除
            from .realistic_sampler import get_realistic_transductive_sampler
            return get_realistic_transductive_sampler(
                dataset=dataset,
...
class DistributedCategoriesSampler(Sampler):
    ...
    def __init__(
        ...
        Args:
            ...
            image_num (int): FSL setting.
        """
        super(DistributedCategoriesSampler, self).__init__()
