...
class Trainer(object):
    ...
    def _validate(self, epoch_idx, is_test=False):
        ...
        with torch.set_grad_enabled(enable_grad):
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                if ((batch_idx + 1) * log_scale % self.config["log_interval"] == 0) or (
                    batch_idx + 1
                ) * episode_size >= max_len * log_scale:
                    info_str = (
                        "Epoch-({}): [{}/{}]\t"
                        "Time {:.3f} ({:.3f})\t"
                        "Calc {:.3f} ({:.3f})\t"
    ...
    def _init_model(self, config):
        ...
        if self.config["pretrain_path"] is not None:
            ...
            all_state_dict = torch.load(self.config["pretrain_path"], map_location="cpu")

            # 1. 尝试从常用的键中提取真正的 state_dict
            if isinstance(all_state_dict, dict):
                if "state_dict" in all_state_dict:
                    state_dict = all_state_dict["state_dict"]
                elif "model" in all_state_dict:
                    state_dict = all_state_dict["model"]
                elif "params" in all_state_dict:
                    state_dict = all_state_dict["params"]
                else:
            ...
            for k, v in state_dict.items():
                name = k.replace("module.", "")  # 移除前缀

                if name.startswith("layer1"):
                    name = name.replace("layer1.0.convShortcut", "layer1.0.shortcut.0")
                elif name.startswith("layer2"):
                    name = name.replace("layer2.0.convShortcut", "layer1.0.shortcut.0")
                elif name.startswith("layer3"):
                    name = name.replace("layer3.0.convShortcut", "layer1.0.shortcut.0")

                new_state_dict[name] = v

            msg = model.emb_func.load_state_dict(new_state_dict, strict=False)
